...
export class InputService {
  ...
  fixCursorPosition(forceToEndPosition?: boolean): void {
    ...
    // if the current cursor position is after the number end position, it is moved to the end of
    // the number, ignoring the prefix or suffix. this behavior can be forced with forceToEndPosition flag
    if (currentCursorPosition > this.getRawValueWithoutSuffixEndPosition() || forceToEndPosition) {
      ...
      // if the current cursor position is before the number start position, it is moved to the
      // start of the number, ignoring the prefix or suffix
